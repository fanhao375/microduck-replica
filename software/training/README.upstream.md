...
### HD1910 replica baseline

The additional `Mjlab-Velocity-{Flat,Rough}-MicroDuck-HD1910` tasks use the
**1910 M6 actuator parameters published by
[LuwuDynamics/xgoduck_rl](https://github.com/LuwuDynamics/xgoduck_rl)** under
Apache-2.0. These parameters were not independently identified by this replica
project. See [parameter provenance](src/mjlab_microduck/robot/hd1910/README.md)
for the fixed source revision, unchanged JSON checksum, and license, and the
[HD1910 usage guide](docs/hd1910-baseline.md) for local verification and commands.
The original Microduck geometry is retained as a simulation baseline; real
replica mass/inertia and hardware validation remain necessary.

### Original Microduck tasks
...
The original Microduck tasks use the [BAM](https://github.com/Rhoban/bam) M6 actuator model for
...
The explicitly named `-HD1910` tasks use LuwuDynamics' public Feetech 1910 M6
parameters instead, with the source and configuration described above.
