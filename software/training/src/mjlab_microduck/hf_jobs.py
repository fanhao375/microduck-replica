...
def _repo_root() -> Path:
    """Training project in the current checkout, which may be a monorepo.

    Resolve from cwd, not the installed module, to snapshot the current
    worktree. Archive paths must start at the training project's pyproject.toml
    because the remote bootstrap runs uv sync at the archive root.
    """
    current = Path.cwd().resolve()
    for candidate in (current, *current.parents):
        if (candidate / "pyproject.toml").is_file() and (
            candidate / "src/mjlab_microduck"
        ).is_dir():
            return candidate
    raise RuntimeError("Run HF Jobs from the training project directory.")


def _build_tarball(repo_root: Path, out_path: Path) -> str:
    """Snapshot this project, including non-ignored new files. Returns Git SHA."""
