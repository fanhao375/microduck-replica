...
| 孔位（板左下角为原点） | MH1 (5.5, 3.5)、MH2 (39.5, 18.5) mm |
| 孔距 | X 向 34.0 mm ／ Y 向 15.0 mm ／ 对角 37.16 mm |

孔位是中心对称的（两孔中点 = 板心 22.5, 11.0），**板子转 180° 装也能对上**。
X 向刻意取 34 mm，跟原版 `banana_pcb_locker` 打印件的两个定位耳间距一致，
这样想沿用原锁扣或改一版都不至于推倒重来。
...
*布线图，由工程实测数据渲染（顶层红 / 底层蓝 / 焊盘黄 / GND 过孔绿 / 信号过孔紫，蓝底为底层 GND 铺铜）。*
*状态：27 器件 · 202 段走线 · 36 过孔，已含 `R7`，`X1`/`R3`/`C9` 已改为实贴。*
*⚠️ 2026-09-08 正在加 `J4`/`J5`（飞特 2.0 mm 座子），此图与下方数据是加它们之前的。*
...
| 项 | 值 |
|---|---|
| 元件 | **27 个**（含 2026-09-07 新增的 R7） |
| 走线 | **207 段**（顶层 186 / 底层 21） |
| 线宽 | 信号 0.25 mm 为主；**总线直通 0.76 mm** |
| 过孔 | **36 个，φ0.6 / 孔 0.3 mm** |
| 铺铜 | **底层整片 GND**，顶层不铺 |

底层只有 19 段走线，**地平面基本完整**——这是给 SPI 和 1 Mbps 半双工总线留的回流路径。
...
|---|---|
| 立创 DRC | **0 违规** |
| 网络连通性 | **22 个网络全部连通**（栅格连通域 + 过孔/插件孔跨层，含底层铺铜） |
| 异网最小间距 | **0.160 mm**。工程规则 0.102 mm、嘉立创双面板下限 0.127 mm，够用 |
| 铜到板边 | 最小 **0.37 mm** |
| M2 让位 | MH1 **3.02 mm** / MH2 **2.95 mm**（离孔心，M2 垫圈半径约 2.5 mm） |
| 过孔规格 | φ0.6 / 孔 0.3 mm，符合嘉立创双面板下限 |
| 去耦距离 | **每个芯片电源脚 2.3 mm 内都有 100nF**：`U1.4`→`C4` 2.27、`U2.5`→`C2` 2.01、`U2.8`→`C1` 2.00、`U3.8`→`C3` 1.96、`U4.3`→`C7` 2.28 mm（`U4.2` LDO 输入侧 3.52 mm，输入电容不挑距离） |

**载流核算**：`J1.2 ↔ J2.2` 直通路径全程 0.76 mm，1 oz 铜下 ΔT=10K 约 2.0 A、
ΔT=30K 约 3.2 A。JST EH 单触点额定 3 A，**走线与连接器基本同一量级**，
加宽走线并不能抬高整条链路的上限，因此维持 0.76 mm。
...
- **连接器高度**：`J1`/`J2`/`J3` 目前是直插（本体约 8 mm）。若躯干内腔确实只有
  6.65 mm，需换卧式 `S3B-EH-A`，封装与 BOM 都要跟着改 —— 用导出的 STEP 在
  装配体里比一下即可定论
...
5. **连接器**：见下方「⚠️ 飞特脚序按型号不同，而且是反的」。**原理图已加** `J4`/`J5`
   两个 2.0 mm 座子对口飞特（2026-09-08，PCB 侧待落位），但**板端座子的确切系列
   还没拿实物比过** —— 飞特规格书只写了线端 `AMP2.0-3P`，板端型号待厂家确认
