...
| 项 | 值 |
|---|---|
| 走线 | **195 段**（顶层 176 / 底层 19），总长 **333.6 mm** |
| 线宽 | 信号 0.25 mm ×164；0.38 ×10；0.51 ×16；**总线直通 0.76 ×5** |
| 过孔 | **34 个，φ0.6 / 孔 0.3 mm** |
| 铺铜 | **底层整片 GND**，顶层不铺 |
...
| 异网最小间距 | **0.153 mm**（`VDD_BUS` 走线 ↔ `J2.1` 的 GND 焊盘）。工程规则 0.102 mm、嘉立创双面板下限 0.127 mm，能过，但这是全板最紧处 |
| 铜到板边 | 最小 **0.37 mm** |
| M2 让位 | MH1 **3.02 mm** / MH2 **2.95 mm**（离孔心，M2 垫圈半径约 2.5 mm） |
| 过孔规格 | φ0.6 / 孔 0.3 mm，符合嘉立创双面板下限 |
| 去耦距离 | **每个芯片电源脚 2.3 mm 内都有 100nF**：`U1.4`→`C4` 2.27、`U2.5`→`C2` 2.01、`U2.8`→`C1` 2.00、`U3.8`→`C3` 1.96、`U4.3`→`C7` 2.28 mm |
