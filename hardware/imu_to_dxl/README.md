...
### 3. J1 / J2 直通，本板最好挂在链条末端

J1 和 J2 三根线一一并联，**本板若在链条中间，下游全部舵机电流都流过本板的连接器和铜箔**。

飞特 HD-1910-C001 官方参数：额定电流 **500 mA** / 堵转 **1.8 A**。15 个舵机额定就是 **7.5 A**，
而 **JST EH 额定只有 3 A**。

→ **建议把本板挂在链条末端**（只用 J1），流过的就只有它自己那 ~12 mA。
若必须置于链中，PCB 上 `VDD_BUS` 和 `GND` 从 J1 到 J2 要走大铜皮并核算下游数量。
