...
- 按访问量定优先级，先翻 `硬件方案逆向.md`（站内访问第 2，84 次），
  ...
  12 字节数据块布局这些只有本仓库有
- 产出：`docs/hardware-teardown.en.md`（426 行），与中文版互相链接
...
- **补充同级别舵机横向对比**：飞特 STS3032（20 g / 23.2×12.1×28.5 mm / 0.44 N·m /
  4.8–6 V）尺寸比 XL330 还小、重量相当，但力矩仅 46% 且电压上限 6 V；
...
- 官方补足：RAM 1GB + 32GB eMMC（**正是 Radxa Zero 3W 的 SKU，反向佐证主控判断**）、
...
### 优先级 中
- [ ] **继续英文化**：`执行器选型.md`（XL330/STS3215/S288 对照表也是独一份）→
      `紧固件反推.md`（仅 102 行）。`社区动态.md` 与 `PROGRESS.md` 不翻 ——
      前者时效性强会过期，后者是内部进度
