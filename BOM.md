...
>
> 核对日期：**2026-09-04**，上游 `microduck_rl` @ `robot_walk.xml`。
...
**舵机占绝对大头。** 按 Robotis 美国官网 $27.49 算，15 个 ≈ **$412**，已经约等于整机
$399 售价；按欧洲零售（€40–46）算则是 **€600–690**，远超。
...
| 摄像头 | 树莓派 Camera v2（**IMX219**） | 1 | `setup-board.sh` 用 `radxa-zero3-rpi-camera-v2` overlay。⚠️ **倒装**，软件里 `rotation=180` |
...
| 尺寸 | 65 × 30 mm，Pi Zero 形态 |
| BOM | 47 行 / **123 颗元件**，其中 **5 行 DNP 不贴** |
| 贴片位 | **117 个**（`POS.csv` 行数） |
...
| C6/C7 | 10 µF 0603 | `C19702` | LDO 输入/输出 |
| R1/R2 | 10 kΩ 0402 | `C25744` | CS 上拉（**必须**）、NRST 上拉 |

> ⚠️ **LDO 选型是个坑**：总线满电 8.4 V，而常见的 AP2112K（6 V）、ME6211（6.5 V）、
> TLV75533（5.5 V）**耐压全部不够，会烧**。必须选 HT7533-1（30 V）或同级。
...
整机 **213 个 M2 过孔**，其中 60 个在舵机本体上（15 × 4）。
